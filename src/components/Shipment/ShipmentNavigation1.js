...
class ShipmentNavigation extends Component {
    ...
    if (value !== null) {
      if (type === "Shipment") {
        if (value.length !== 0) {
          ...
          this.getShipmentListByStatus(query, "Shipment");
        } else {
          this.setState({ shipmentList: [] });
        }
        if (value.length !== 0) {
          if (value[0].value === 'All') {
            var dataset = this.state.StatusList.filter((x) => x.value !== "All");
          ...
          this.setState({ shipmentList: [], shipmentstatusList: value });
        }
        ...
        if (value.length !== 0) {
          ...
          this.getShipmentListByStatus(query, "Myshipment");
        } else {
          this.setState({ myshipmentstatusList: [] });
        }
        if (value.length !== 0) {
          if (value[0].value !== 'All') {
          ...
          this.setState({ myshipmentstatusList: [], shipmentstatusList: value });
        }
      }
    }
  };

  getShipmentListByStatus = (query, type) => {
    try {
      let data = {
        ContactName: "",
        ContactNumber: "",
  ...
  getStatusList = () => {
    try {
      ...
      api.post("stringMap/getstringMap", data)
        .then((result) => {
    ...
    if (value !== null) {
      if (type === "ManagedBy") {
        ...
        if (value.length !== 0) {
          ...
          if (
            CommonConfig.isEmpty(query) &&
            value.findIndex((x) => x.value === "All") !== -1
          ) {
            query = "All";
          }
          this.setState({ StatusQuery: query });
        } else {
          this.setState({ ShipmentStatus: [] });
        }
  ...
  searchrenderInputMethod = (params) => {
    return <TextField {...params} label="Shipment Status1" variant="outlined" />;
  };
                ...
                <div className="filter-top-right">
                  <div className="autocomplete-fs-small">
                    <Autocomplete
                      ...
                      value={shipmentstatusList}
                      getOptionLabel={(option) => option.label}
                      onChange={(event, value) =>
                        this.shipmentStatusChange(event, value, "Shipment")
                      }
                      style={{ width: "100%" }}
                      ...
                      // renderOption={(option ) => this.shipmentrenderOption(option)}
                      renderInput={(params) =>
                        this.shipmentrenderInputMethod(params)
                      }
                    />
                  </div>
                </div>
                ...
                <div className="filter-top-right">
                  <div className="autocomplete-fs-small">
                    <Autocomplete
                      ...
                      id="filtercheckbox"
                      options={this.state.StatusList}
                      value={myshipmentstatusList}
                      getOptionLabel={(option) => option.label}
                      onChange={(event, value) =>
                        this.shipmentStatusChange(event, value, "Myshipment")
                      }
                      style={{ width: "100%" }}
                      renderOption={(option) =>
                        this.myshipmentrenderOption(option)
                      }
                      renderInput={(params) =>
                        this.myshipmentrenderInputMethod(params)
                      }
                    />
                  </div>
                </div>
