...
class ShipmentNavigation extends Component {
  ...
  async componentDidMount() {
    ...
    if (
      ...
      if (this.props.history.location.state.type === "Shipment") {
        ...
        // this.getShipmentListByStatus(this.state.shipmentquery, "Shipment")
        APIcheck = false;
      }
    } else {
      var finalSort = {
    ...
    if (APIcheck) {
      ...
      // let newFilter = [{ label: "Select Filter", value: "Select Filter" }];
    ...
    if (
      ...
      if (this.props.history.location.state.type === "MyShipment") {
        ...
        // this.getShipmentListByStatus(this.state.shipmentquery, "MyShipment")
        myAPIcheck = false;
      }
    } else {
      var finalSort = {
    ...
    if (myAPIcheck) {
      ...
      // let allFilter = [{ label: "Select Filter", value: "Select Filter" }];
  ...
  getShipmentListByStatus = (query, type) => {
    try {
      ...
      if (inputdata.length === 0 || inputdata === "All") {
        Query = inputdata;
        ...
        for (var j = 0; j < inputdata.length; j++) {
          ...
          //  else if (j + 1 === 1) {
          //   Query = Query + ` OR sm.ShipmentStatus = "` + inputdata[j].value + `")`;
          // }
          else {
            Query =
  ...
  showSearchFilter = (type) => {
    ...
    this.getShipmentListByStatus(this.state.StatusQuery, type); // "Shipment"
  };
  ...
  handleCheckboxChange = (e, record, type) => {
    ...
    if (type !== "All") {
      ...
      // var checkdata = checkedArr.filter((x) => x.IsSelected === true);
      // for (var i = 0; i < checkedArr.length; i++) {
      //if (i === 0) {
      ////////////////vivek//////////////
      // if (checkedArr[record.Index]["IsSelected"] === true) {
      //   if (CommonConfig.isEmpty(this.state.StatusQuery)) {
      //     StatusQuery = ` AND ( sm.ShipmentStatus = "` + type + `")`;
      //     this.state.shipmentquery = this.state.shipmentquery + StatusQuery;
      //   }
      // } else {
      ...
      // if (checkedArr[record.Index]["IsSelected"] === true) {
      //   if (CommonConfig.isEmpty(this.state.StatusQuery)) {
      //     StatusQuery = ` OR ( sm.ShipmentStatus = "` + type + `")`;
      //     this.state.shipmentquery = this.state.shipmentquery + StatusQuery;
      //   }
      // } else {
      //   this.setState({ shipmentquery: "" });
      ...
      // else{
      //   console.log('this.state.StatusQuery: ',this.state.StatusQuery);
      // }
    } else {
      debugger;
      // StatusQuery = '';
              ...
              this.state.Steps[
                ...
                <div className="filter-top-right">
                  <Button
                    className="cm-toggle"
                    color="rose"
                    onClick={() => this.setState({ IsDropDownShow: true })}
                  >
                    Shipment Filter
                  </Button>
